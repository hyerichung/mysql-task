...
* ### Safari에서 오디오 재생 관리
  ...
  - 해당 이슈는 safari에서 audio 재생 시 '미리 로딩 완료 된' 음원만 재생 가능한데, 저의 경우 useEffect를 사용하여 audio.current.play()를 호출하고 있기 때문에 문제가 발생하였습니다.
    - useEffect는 화면이 렌더링, 페인팅 완료 된 이후 실행되기 때문에 '미리 로딩 되지 않은' 오디오를 먼저 play() 하게 되어 문제가 발생합니다.
    - 따라서 useLayoutEffect를 사용하여 화면이 페인팅 되기 전 미리 로직을 실행하는 방식으로 변경하여 해당 이슈를 해결했습니다.
